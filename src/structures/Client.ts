...
export class Client {
	 ...
	 */
	public constructor(private token: string, options?: ClientOptions) {
	...
	private async request(method: HTTPMethod, path: string, params: any = {}, query: any = {}): Promise<any> {
		const init = req('https://api.treefarmer.xyz', method).path(path).header('Authorization', this.token).body(params);
